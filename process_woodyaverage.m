function varargout = process_woodyaverage( varargin )
...
%  - modified from process_average.m of the Brainstorm software
...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.Comment     = 'Woody-Average files';
    sProcess.Category    = 'Custom';
    sProcess.SubGroup    = {'CPL', 'Average'};
    sProcess.Index       = 302;
    ...
    % === WOODY FILTER OPTIONS
    sProcess.options.label3.Comment = '<U><B>Woody Options</B></U>:';
    sProcess.options.label3.Type    = 'label';
    sProcess.options.woodytype.Comment = {'Single Channel:  <FONT color="#777777">mean(x)</FONT>', ...
                                         'All Channels Simultaneously:  <FONT color="#777777">mean(abs(x))</FONT>', ...
                                         'All Channels Individually:  <FONT color="#777777">sqrt(sum(x.^2)/N)</FONT>'};
    sProcess.options.woodytype.Type    = 'radio';
    sProcess.options.woodytype.Value   = 1;
    sProcess.options.woodychannel.Comment = 'Channel to use for Woody Filter';
    sProcess.options.woodychannel.Type = 'channelname';
    sProcess.options.woodychannel.Value = [];
    sProcess.options.label4.Comment = '<U><B>Averaging Options</B></U>:';
...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    % Woody Options
    if isfield(sProcess.options, 'woodytype') && isfield(sProcess.options.woodytype, 'Value') && ~isempty(sProcess.options.woodytype.Value)
        woody_type = sProcess.options.woodytype.Value;
    else
        woody_type = 1;
    end
