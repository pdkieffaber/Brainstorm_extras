function varargout = process_ERPplot( varargin )
% PROCESS_WOODYAVERAGE: Average files, by subject, by condition, or all at once using a Woody Filter.
%
% USAGE:                    OutputFiles = process_woodyaverage('Run', sProcess, sInputs)
%                            OutputFile = process_woodyaverage('AverageFiles', sProcess, sInputs, KeepEvents, isScaleDspm, isWeighted, isMatchRows, isZeroBad)
%                        [sMat,isFixed] = process_woodyaverage('FixWarpedSurfaceFile', sMat, sInput, sStudyDest)
%  [iGroups, GroupComments, GroupNames] = process_woodyaverage('SortFiles', sInputs, avgtype)

% @=============================================================================
% This function is part of the CPL Toolbox written for Brainstorm software:
% https://neuroimage.usc.edu/brainstorm
% 
% Copyright (c)2020 College of William and Mary
...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    % === CHANNEL SELECTION
    sProcess.options.chanselect.Comment = 'Select a channel to plot...';
    sProcess.options.chanselect.Type = 'channelname';
    sProcess.options.chanselect.Value = ' ';
    ...
    %
    sProcess.options.command.Comment = 'Custom Commands (run at end of process)';
    sProcess.options.command.Type = 'textarea';
    sProcess.options.command.Value = ' ';
    %
    sProcess.options.plotrange.Comment = 'Time Range to Plot';
    sProcess.options.plotrange.Type = 'timewindow';
    sProcess.options.plotrange.Value = [];
end
...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    if bst_progress('isVisible')
        ...
        bst_progress('text', 'Generating Plot...');
    else
        curProgress = [];
    end
...
%     if woody_type==1 && isEmpty(sProcess.options.woodychannel.Value)
%         bst_report('Error', sProcess, [], 'No channel was identified for Woody-filter.');
%         OutputFiles = [];
%         return;
%     end
    ...
    figure;
    hold on
    plot([-1200, 2000],[0, 0],'k:','LineWidth',2)
    plot([0, 0],[-30,30],'k:','LineWidth',2)
    ymax=0;
    ymin=0;
    %LOOP OVER FILES
    for i = 1:length(iGroups)
        %[sStudy, iStudy, iData] = bst_get('DataFile', sInputs(i).FileName);
        ChannelFile = sInputs(i).ChannelFile;
        ChannelMat = in_bst_channel(ChannelFile);
        thischan=find(strcmp({ChannelMat.Channel.Name},sProcess.options.chanselect.Value));
        % Load file
        DataMat = in_bst_data(sInputs(i).FileName, 'F', 'ChannelFlag', 'History', 'Time');
        h(i)=plot(DataMat.Time*millisecond,DataMat.F(thischan,:)*volt,'LineWidth',4);
        sInputs(i)
        DataMat
        set(gcf,'color','w')
        ...
        if min(DataMat.F(thischan,:))<ymin
            ymin=min(DataMat.F(thischan,:));
        end
        if max(DataMat.F(thischan,:))>ymax
            ymax=max(DataMat.F(thischan,:));
        end
        % Compute group average
        %OutputFiles{end+1} = AverageFiles(sProcess, sInputs(iGroups{i}), KeepEvents, isScaleDspm, isWeighted, isMatchRows, isZeroBad);
    end
    Xlim=[sProcess.options.plotrange.Value{1}*millisecond];
    Ylim=[ymin*volt ymax*volt];
    set(gca,'fontsize',FontSize,'xlim',Xlim,'ylim',Ylim)
    eval(sProcess.options.command.Value)
...
function OutputFile = AverageFiles(sProcess, sInputs, KeepEvents, isScaleDspm, isWeighted, isMatchRows, isZeroBad)
    ...
    if isfield(sProcess.options, 'avg_func') && isfield(sProcess.options.avg_func, 'Value') && ~isempty(sProcess.options.avg_func.Value)
        switch (sProcess.options.avg_func.Value)
            ...
            case 7,  Function = 'mean';   isbVariance = 1;   strComment = 'AvgStderr';
