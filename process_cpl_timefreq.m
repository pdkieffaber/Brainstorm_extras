function varargout = process_cpl_timefreq( varargin )
...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.Comment     = 'Time-frequency (Morlet wavelets) CPL';
    sProcess.Category    = 'Custom';
    sProcess.SubGroup    = 'CPL';
    sProcess.Index       = 505;
    ...
    sProcess.options.label2.Comment = '(Ignored if saving individual time-frequency maps)';
...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    switch (func2str(sProcess.Function))
        case 'process_cpl_timefreq',  strProcess = 'morlet';
    ...
    % === SORT DATA FILES BY GROUPING OPTION===
    [iGroups, GroupComments] = SortFiles(sInputs, sProcess.options.avgtype.Value);
    
    % === RUN TF FOR EACH GROUP===
    for i = 1:length(iGroups)    
        % === OUTPUT STUDY ===
        if strcmpi(tfOPTIONS.Output, 'average')
            % Get output study
            [sStudy, iStudy, Comment] = bst_process('GetOutputStudy', sProcess, sInputs(iGroups{i}));
        ...
        tfINPUT=DataToProcess(iGroups{i});
        % Compute group average
        % === START COMPUTATION ===
        [OutputFiles, Messages, isError] = bst_timefreq(tfINPUT, tfOPTIONS);
        if ~isempty(Messages)
            if isError
